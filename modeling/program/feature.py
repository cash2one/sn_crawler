	...
	with open(outputPath+featureFileName, "w") as fo:
		for i in range(len(gts)):
			...
			for j in range(len(gts)):
				...
				outputStr = str(rank)+" qid:"+users_sn1[i]
				for k in range(len(scores)):
					score = scores[k]
					outputStr += " " + str(k+1)+":"+str(score)
				outputStr += " # "+users_sn2[j]+"\n"
				fo.write(outputStr)
	e = time.time()
...
def getScores(sn1, user1, sn2, user2, g1, g2, g0):
	...
	if sn2 == "twitter":
		user2_name = twitterNameId[user2]
		...
		user1_name = twitterNameId[user1]
...
def buildGraphs():
	# google graph
	print("google graph")
	googleDiGraph = buildDiGraph(inputPath+"google/relationship_file")
	# twitter graph 
	print("twitter graph")
	twitterDiGraph = buildDiGraph(inputPath+"twitter/relationship_file_revise")
	# full graph
	print("full graph")
...
def calTemporalScore(times1, times2, post_with_place_index1, post_with_place_index2, posts1, posts2):
	...
	temp_topic_distri1_day, temp_topic_distri1_week, temp_topic_distri1_mon = temp_topic_distri(posts1, mons1, weeks1, days1)
	temp_topic_distri2_day, temp_topic_distri2_week, temp_topic_distri2_mon = temp_topic_distri(posts2, mons2, weeks2, days2)
